...
**快照日期**：2026-04-24（v0.1.0 发布前更新，后续应按 `AGENTS.md` 定期刷新）
...
近期后端增强（面向生产集成）：`GET /readyz`（数据库就绪探针）、`DefaultBodyLimit` 限制摄入请求体、默认关闭未配置 Langfuse 密钥时的匿名兼容路径（`XTRACE_ALLOW_UNAUTHENTICATED_COMPAT`）、trace 列表响应补充 `projectId` / `createdAt` 等与 Langfuse 对齐的字段。v0.1.0 这轮还补充了 Xinference 对接文档、GCP/systemd 部署样例、联调脚本，以及 batch ingest 对 `sessionId` 的兼容别名解析。
